...
export default function VoiceRegistrationPage() {
  ...
  const [currentField, setCurrentField] =
    useState<RegistrationField>("farm_name");
  const [question, setQuestion] = useState("");

  const [farmState, setFarmState] =
    useState<VoiceFarmState>(getEmptyState());
  ...
  const speakCurrentQuestion = useCallback(
    (field: RegistrationField, text: string) => {
      cancelSpeech();
      stopTamilQuestion();
    ...
    try {
      ...
      speakTimerRef.current = window.setTimeout(() => {
        speakCurrentQuestion(
          response.current_field as RegistrationField,
          response.question
        );
  ...
  const processAnswer = useCallback(
    async (transcript: string) => {
      ...
      try {
        ...
        // Save completed voice registration locally
        if (response.complete) {
          localStorage.setItem(
            "vazhaiguard_farm_profile",
            JSON.stringify(response.farm_state)
          );
        }
        ...
        setTimeout(() => {
          if (language === "ta-IN") {
            // Play Tamil MP3 for the next field
            if (
              response.next_field &&
              response.next_field !== "complete"
            ) {
              void playTamilAcceptedTurn(
                response.next_field
              ).catch((error) => {
                console.error(
                  "[TamilVoice] Accepted turn audio failed:",
                  error
                );
              });
            } else if (
              response.current_field &&
              response.current_field !== "complete"
            ) {
              void playTamilRetry(
                response.current_field
              ).catch((error) => {
                console.error(
                  "[TamilVoice] Retry audio failed:",
                  error
                );
              });
            }
  ...
  const beginListening = () => {
    ...
    startListening(language, (finalTranscript) =>
      void processAnswer(finalTranscript)
    );
  };
    ...
    <div className="mx-auto w-full max-w-[1350px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
        <VoiceAssistantCard
          ...
          onReplay={() =>
            speakCurrentQuestion(currentField, question)
          }
        />
        ...
        <aside className="vg-card p-5 sm:p-6">
          ...
          <div className="mt-5 space-y-2.5">
            <ProfileValue
              ...
              value={farmState?.farm_name}
            />

            <ProfileValue
              label="மொத்த நிலப்பரப்பு"
              value={
                farmState?.total_farm_acres
            ...
            <ProfileValue
              label="வாழை வகை"
              value={farmState?.banana_variety}
            />

            <ProfileValue
              label="வடிகால்"
              value={farmState?.drainage}
            />

            <ProfileValue
              label="ஆதரவு கம்பு"
              value={farmState?.support}
            />

            <ProfileValue
              label="அணுகல்"
              value={farmState?.accessibility}
            />
          </div>
          ...
          {!showTyping ? (
            ...
            <div className="vg-card flex flex-col gap-3 p-4 sm:flex-row">
              ...
              <Button
                ...
                className="h-11 rounded-xl bg-[#073b2a]"
              >
                அனுப்பு
              </Button>
            </div>
